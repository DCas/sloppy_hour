class ScheduleInput < SimpleForm::Inputs::Base
  ...
  def input
    ...
    @builder.simple_fields_for(:schedule, @builder.object.schedule || @builder.object.build_schedule) do |b|
      ...
      b.template.content_tag("div", {id: b.object_name.to_s.gsub(/\]\[|[^-a-zA-Z0-9:.]/,"_").sub(/_$/,"")}) do
        ...
        b.input(:rule, collection: ['singular', 'daily', 'weekly', 'monthly'], label_method: lambda { |i| I18n.t("schedulable.rules.#{i}") || i.capitalize }, label: false, include_blank: true, selected: ['']) << 
        ...
        template.content_tag("div", {data: {group: 'weekly'}}) do
          b.input :days, collection: Time::DAYS_INTO_WEEK.invert.values, label_method: lambda { |v| I18n.t("date.day_names")[days_after_monday[v]] || v.capitalize }, as: :check_boxes
        end <<
                ...
                Time::DAYS_INTO_WEEK.invert.values.reduce(''.html_safe) { | x, weekday | 
                  x << template.content_tag("tr") do 
                    template.content_tag("td") do
                      db.label I18n.t("date.day_names")[days_after_monday[weekday]] || weekday, required: false
                    end << 
        ...
        (template.content_tag("div", {data: {group: 'daily,weekly,monthly'}}) do
            b.input :until, default: 1.year.from_now
  ...
  def days_after_monday
    Time::DAYS_INTO_WEEK.inject({}) { |h, (k, v)| h[k] = (v+1)%7; h }
  end
